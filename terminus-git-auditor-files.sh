...
UPSTREAM_BRANCH='master' #$(echo ${UPSTREAM} | jq -r .repository_branch)
CURRENT_DATETIME=$(date +"%Y-%m-%d-%H-%M-%S")

# Set audit file name / path.
AUDIT_FILE=$(echo "/tmp/$UPSTREAM_NAME-git-audit-$CURRENT_DATETIME.csv")
...
touch $AUDIT_FILE
echo "id,name,upstream,upstream_status,file_diff_count,diff_status" >> $AUDIT_FILE

# Generate a list of all files that are not ignored by git in the local repository
upstream_files=$(git ls-files --exclude-standard)
...
jq -n "$SITES" | jq '. | to_entries | .[].key' | while read i; do
    ...
    # Checkout the default branch of the remote repository
    git checkout ${SITE_NAME}/master

    # Generate a list of all files that are not ignored by git in the remote repository
    site_files=$(git ls-files --exclude-standard)

    # Find the files that are different between the two repositories
    different_files=$(comm -3 <(echo "$upstream_files" | sort) <(echo "$site_files" | sort))
    different_count=$(echo "$different_files" | wc -l)

    # Print the files that are different between the two repositories
    echo "Files that are different between upstream and site repositories:"
    echo "$different_files"

    echo "Number of files that are different: ${yellow}$different_count${reset}"
    echo ""
    ...
    echo "${SITE_ID},${SITE_NAME},${UPSTREAM_NAME},${UPSTREAM_STATUS},${different_count},${DIFF_STATUS}" >> $AUDIT_FILE
done
